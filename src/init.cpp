 ...
 */
static const char* BITCOIN_PID_FILENAME = "wojakcoind.pid";
...
void SetupServerArgs(NodeContext& node)
{
    ...
    argsman.AddArg("-mineraddress=<addr>", "Send miner portion of block reward to this address when the development fund is active. Required for getblocktemplate when dev fund is active and wallet is disabled.", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/wojak/wojakcore>";
...
bool AppInitMain(const util::Ref& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if wojakcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if wojakcoin is started while in a temporary directory.\n",
